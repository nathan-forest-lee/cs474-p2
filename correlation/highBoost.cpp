...
int rows, cols, Q, val, maskVal, initVal, smoothVal;
...
void unsharpMasking(ImageType &initImage, ImageType &outputImage, ImageType &unsharpImage)
{
    for (int i = 0; i < rows; i++)
    {
        for (int j = 0; j < cols; j++)
        {
            initImage.getPixelVal(i, j, val);
            initVal = val;
            val = 0;
            outputImage.getPixelVal(i, j, val);
            smoothVal = initVal - val;
            unsharpImage.setPixelVal(i, j, smoothVal);
        }
    }
}

void highBoost(ImageType &initImage, ImageType &highpassImg, ImageType &highBoostImage)
{
    const float A = 1.7;
    for (int i = 0; i < rows; i++)
    {
        for (int j = 0; j < cols; j++)
        {
            ...
            initVal = val * (A - 1);
            val = 0;
            highpassImg.getPixelVal(i, j, val);
            smoothVal = initVal + val;
            highBoostImage.setPixelVal(i, j, smoothVal);
        }
    }
}
...
int main(int argc, char *argv[])
{
    ...
    ImageType unsharpImage(rows, cols, Q);
    ImageType highBoostImage(rows, cols, Q);

    smooth(initImage, 7, 7, outputImage);

    unsharpMasking(initImage, outputImage, unsharpImage);
    highBoost(initImage, unsharpImage, highBoostImage);

    writeImage(argv[2], highBoostImage);
